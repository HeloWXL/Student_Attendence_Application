    ...
    $.ajax({
        ...
        success:function (data) {
            ...
            for(var i = 0 ; i<data.data.length;i++){
                var $node = $(' <li name = "'+data.data[i].signId+'" class="mui-table-view-cell mui-collapse" onclick="toAttencedetailView(this)" >\n' +
                    '            <a>'+data.data[i].startTime+'<span class="mui-icon mui-icon-arrowright"></span></a>\n' +
                    '        </li>')
